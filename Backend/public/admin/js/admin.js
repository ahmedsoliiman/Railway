...
async function loadDashboardStats() {
    try {
        ...
        if (data.success) {
            ...
            document.getElementById('stat-tours').textContent = stats.active_tours;
        ...
        if (data.success) {
            ...
            data.data.tours.forEach((tour) => {
                ...
                const row = `
                    <tr>
                        <td>${tour.id}</td>
                        <td>${tour.train_name} (${tour.train_number})</td>
                        <td>${tour.origin_city} → ${tour.destination_city}</td>
                        <td>${new Date(tour.departure_time).toLocaleString()}</td>
                        <td>${new Date(tour.arrival_time).toLocaleString()}</td>
...
async function editTour(id) {
    ...
    try {
        ...
        if (data.success) {
            const tour = data.data.tours.find((t) => t.id === id);
            if (tour) {
                document.getElementById('tour-modal-title').textContent = 'Edit Tour';
                document.getElementById('tour-id').value = tour.id;
                document.getElementById('tour-train').value = tour.train_id;
                document.getElementById('tour-origin').value = tour.origin_station_id;
                document.getElementById('tour-destination').value = tour.destination_station_id;
                document.getElementById('tour-departure').value = new Date(tour.departure_time).toISOString().slice(0, 16);
                document.getElementById('tour-arrival').value = new Date(tour.arrival_time).toISOString().slice(0, 16);
                document.getElementById('tour-first-price').value = tour.first_class_price;
                document.getElementById('tour-second-price').value = tour.second_class_price;
                document.getElementById('tour-seats').value = tour.available_seats;
...
document.getElementById('tour-form')?.addEventListener('submit', async (e) => {
    ...
    try {
        const url = id ? `${API_URL}/admin/tours/${id}` : `${API_URL}/admin/tours`;
        const method = id ? 'PUT' : 'POST';
